...
and returns the token part of the header
'''
...
'''
The check_permissions(permission, payload) method
    @INPUTS
    ...
    payload permissions array, and returns true otherwise
'''
...
'''
The verify_decode_jwt(token) method
    @INPUTS
        token: a json web token (string)

    Tt verifies that token is an Auth0 token with key id (kid)
    and uses Auth0 /.well-known/jwks.json
...
'''
The @requires_auth(permission) decorator method
    @INPUTS
        permission: string permission (i.e. 'post:drink')
